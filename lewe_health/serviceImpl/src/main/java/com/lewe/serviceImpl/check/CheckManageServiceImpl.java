...
public class CheckManageServiceImpl implements ICheckManageService{
	...
	public JSONObject checkSampleList(String sampleInfoQuery, Account loginAccount, Object apiResult) {
		...
		for (ReportInfo reportInfo : list) {
			...
			sampleInfo.put("submitTime", reportInfo.getSubmitTime()==null?null:DateUtil.formatDate(reportInfo.getSubmitTime(), "yyyy-MM-dd"));//采样提交时间
			sampleInfo.put("checkTime", reportInfo.getCheckTime()==null?null:DateUtil.formatDate(reportInfo.getCheckTime(), "yyyy-MM-dd"));//检测日期
			sampleInfo.put("hospitalScanTime", reportInfo.getHospitalScanTime()==null?null:DateUtil.formatDate(reportInfo.getHospitalScanTime(), "yyyy-MM-dd"));//门店扫描日期
	...
	public JSONObject checkInfoDetail(Long id,String sampleCode, Account loginAccount, Object apiResult) {
		...
		if(reportInfo!=null) {
			...
			List<CheckItemSubstrate> substrateList = checkItemSubstrateMapper.selectListByItemId(reportInfo.getCheckItemId());
	...
	public int submitCheckInfo(String checkInfo, Account loginAccount, Object apiResult) {
		if(StringUtils.isNotBlank(checkInfo)) {
			...
			if(hospital!=null && checkDevice!=null) {
				...
				sysReportCode = hospital.getHospitalCode()+checkDevice.getCode()+DateUtil.formatDate(new Date(), "yyyyMMdd")+flowCode;
			}
			...
			if(json!=null) {
				...
				if(sampleAge<18) {//小于18岁时的文案(儿童版)
					if(reportResult==5) {
						reportResultDescription = "儿童阴性：健康的肠道菌群有助于孩子的消化吸收、排毒通便、生长发育、加强免疫力等";
					...
					if(reportResult==5) {
						reportResultDescription = "成人阴性：健康的肠道菌群有助于我们的消化吸收、排毒通便、增强免疫力等";
					}else if(reportResult==1) {
	...
	public JSONObject checkAuditList(String sampleInfoQuery, Account loginAccount, Object apiResult) {
		...
		for (ReportInfo reportInfo : list) {
			...
			auditInfo.put("submitTime", reportInfo.getSubmitTime()==null?null:DateUtil.formatDate(reportInfo.getSubmitTime(), "yyyy-MM-dd"));//采样提交时间
			auditInfo.put("checkTime", reportInfo.getCheckTime()==null?null:DateUtil.formatDate(reportInfo.getCheckTime(), "yyyy-MM-dd"));//检测日期
			...
			auditInfo.put("auditTime", reportInfo.getAuditTime()==null?null:DateUtil.formatDate(reportInfo.getAuditTime(), "yyyy-MM-dd"));//审核时间
