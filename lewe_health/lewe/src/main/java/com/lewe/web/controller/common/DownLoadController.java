...
public class DownLoadController {
	...
	public void downLoadReport(HttpServletRequest request, Long reportInfoId, String reportInfoIds, Integer type,
		...
		if (type == null) {
			...
			if (StringUtils.isNotBlank(reportInfoIds)) {
				...
				if (type == 1) {// 下载PDF
					...
					if (sysfile != null) {
						...
						try {
							String downFileName = new String(
									(reportInfo.getSampleName() + "的检测报告PDF文件.pdf").getBytes("gbk"), "iso8859-1");
							response.setHeader("Content-Disposition", "attachment; filename=" + downFileName);
							...
							if (url.contains("http")) {
								...
								/*
								 * InputStream inStream = UrlUtil.urlToIO(url); int len; byte[] byteArr = new
								 * byte[1024]; while ((len = inStream.read(byteArr)) > 0) {
								 * response.getOutputStream().write(byteArr, 0, len); } inStream.close();
								 */

								String domainName = PropertiesUtil.getApiPropertyByKey("domain.name");
								// 前端项目路径
								String leweWebPath = PropertiesUtil.getApiPropertyByKey("lewe.web.path");
								SysFile pdfFile = sysFileMapper
										.selectByPrimaryKey(Long.valueOf(reportInfo.getReportPdfIds()));
								url = pdfFile.getUrl();
								// 将文件的网络路径转换成本地路径读取
								if (url.startsWith(domainName)) {
					...
					String downFileName = new String((reportInfo.getSampleName() + "的检测报告图片.zip").getBytes("gbk"),
							"iso8859-1");
					response.setHeader("Content-Disposition", "attachment; filename=" + downFileName);
					ZipOutputStream zipOut = new ZipOutputStream(response.getOutputStream());
					String domainName = PropertiesUtil.getApiPropertyByKey("domain.name");
