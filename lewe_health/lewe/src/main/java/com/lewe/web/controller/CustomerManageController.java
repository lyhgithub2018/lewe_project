...
public class CustomerManageController extends BaseController {
	...
	public ApiResult addOrUpdateCustomerInfo(HttpServletRequest request, Integer operateType,
	...
	public ApiResult deleteMy(HttpServletRequest request, Long id, HttpServletResponse response) {
		ApiResult result = new ApiResult();
		Account loginAccount = getSessionAccount(request, result);
		if (loginAccount != null) {
			result.setData(reportInfoMapper.deleteByPrimaryKey(id));
		}
		return result;
	}
	...
	public ApiResult addExpressInfo(HttpServletRequest request, String expressName, String expressCode,
			String customerInfoIds, Integer channelId, Long hospitalId, HttpServletResponse response) {
		ApiResult result = new ApiResult();
		Account loginAccount = getSessionAccount(request, result);
		if (loginAccount != null) {
			customerManageService.addExpressInfo(expressName, expressCode, customerInfoIds, channelId, hospitalId,
					loginAccount, result);
		}
		return result;
	}
	...
	public ApiResult expressInfoList(HttpServletRequest request, String keyword, Integer channelId, Integer pageNo,
			Integer pageSize, HttpServletResponse response) {
		ApiResult result = new ApiResult();
		Account loginAccount = getSessionAccount(request, result);
		if (loginAccount != null) {
			JSONObject json = customerManageService.expressInfoList(keyword, channelId, pageNo, pageSize, loginAccount,
					result);
			result.setData(json);
		}
		return result;
